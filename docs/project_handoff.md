...
## 2026-07-13：16 位精度数据验证——方法正确性盖章，全部 workaround 可退役

对方重导了 16 位有效数字的三个文件（ssor_k1/ssor_k2/right hand.txt，放于
`~/ams_example_16`，索引/拓扑文件精度无关、从旧目录软链）。三组对照实验
（Run A 全 workaround / Run B 无 β workaround / Run C 探地板，命令与日志见
`~/run_16a/b/c.log`）+ `scripts/solution_forensics.py` 代回法证，结论：

| 数据 | 求解 rtol | 外层步数 | AMS 代回残差 | slove.txt 代回残差 | 高 σ 区两解一致性 |
| --- | --- | --- | --- | --- | --- |
| 8 位 | 1e-7 | 44 | 6.8e-8 | 6.8e-7 | 3.2e-5 |
| 16 位 | 1e-7 | 19 | 2.8e-7 | 5.2e-7 | 1.3e-7 |
| 16 位 | 1e-12 | 34 | **3.9e-12** | 5.2e-7 | 1.5e-8 |

1. **不存在收敛地板**：16 位数据下 34 步收敛到 5.0e-13（内部相对残差），代回残差
   3.9e-12。旧的 ~1e-7 极限纯属 8 位导出精度，与方法无关。"根本不收敛说明解法有错"
   的担忧在本数据上与事实相反——AMS 预条件收敛性极佳且比直接解精确 5 个量级。
2. **β 两件 workaround 在 16 位数据下不再需要**：去掉 `-ams_beta_mass_poisson`
   与 `-ams_beta_mass_shift` 后收敛行为逐位一致（19 步、无 NaN）。它们只是在补偿
   8 位消去噪声（GᵀKrG 残留 > 空气 σ 信号），与根因诊断完全一致。8 位数据仍需保留。
3. **规范自由度假象再次证实**：与 slove.txt 的全局 L2 差仍在（0.36~0.40，数值随
   规范漂移），差异能量 100.0% 在低 σ 边；而高 σ（可观测）区一致性随精度从 3.2e-5
   提升到 1.5e-8——两个解在物理上是同一个解，全局 L2 仍是病态验收标准。
4. `-fortran_upper_unscale_edge_len`（边长反缩放）在 16 位数据下依然必需：这是
   基函数约定差异，与精度无关。
5. 16 位数据的推荐命令（在旧标准命令上去掉两个 β 选项即可）：
   ```bash
   ./build_ubuntu/UpperExampleCheck ~/ams_example_16 \
     -fortran_upper_unscale_edge_len -B_ksp_type gmres -B_ksp_max_it 15 \
     -em_outer_max_it 60 -em_outer_rtol 1e-7 -A_ksp_gmres_restart 15
   ```
   墙钟 ~1.5 分钟（文件已入页缓存时；冷缓存加 ~5 分钟解析）。
6. 备注：slove.txt 仍是 8 位旧导出（代回 16 位矩阵只到 5.2e-7）；若对方连直接解也
   16 位重导，对照会更干净，但已非必需——代回残差已足以验收。
7. 对方提到有论文称"此迭代解法若根本不收敛则解法有错"（具体文献待要）：拿到
   文献后核对其针对的公式化（复对称系统的实块等价形式谱性质？），但以上实测
   表明在正确基函数约定 + 足够数据精度下本实现收敛到机器精度附近。

### 补充实验（2026-07-13 深夜）：0.36 全局差的终审证据 + β-shift 深容差地板

**同一求解器、两种 PC 配置（干净 AMS vs β-mass-shift），都探 rtol 1e-12**：

| 解 | 代回残差 | 相互全局 L2 差 | 差异能量在低 σ 边 | 高 σ 区相互差 |
| --- | --- | --- | --- | --- |
| 干净配置（34 步收敛） | 3.9e-12 | —— | —— | —— |
| β-shift 配置（120 步封顶） | 4.7e-11 | **0.19（vs 干净配置）** | 100.0% | **2.3e-11** |
| slove.txt（直接解） | 5.2e-7 | 0.36 / 0.40（vs 上两者） | 100.0% | 1.5e-8 |

**解读：我们自己的两个解都把方程满足到 ≤5e-11，彼此的全局 L2 差却有 0.19**，且
100% 在空气边、导电区一致到 2.3e-11。这证明空气层棱边分量在任何可达精度下都不由
线性系统决定（近零特征值 × 有限残差 = O(1) 分量不确定性）——**全局 L2 会把同一个
求解器自己判成"自相矛盾 19%"，它不度量解的质量**。直接解只是同一物理解的第三个
任意代表（导电区与我们两解各差 1.5e-8，恰为其自身残差 5.2e-7 决定的水平）。
若对方坚持空气区数值一致，唯一途径是规范固定（对方告知约定或解后投影），属后处理。

**β-shift 的深容差地板（新发现）**：β-mass-shift 配置到 1e-7 无碍（19 步），但深于
~1e-9 后速率崩塌——120 步封顶仅到 4.7e-11，而干净配置 34 步到 3.9e-12。PC 里对 M
的 1e-6 相对位移即 PC 质量的地板。结论加强：16 位数据下两个 β workaround 应退役。

**Run C 收敛形态记录**（16 位、干净配置、rtol 1e-12）：外层稳定线性收敛约每 2 步一个
数量级（成对下降=复共轭特征值对的实块指纹）；restart 15 在第 15/30 步各致 ~2 步平台
后恢复；内层 AMS B-solve 除首次外全部 3~4 步收敛（68 次调用）。

**关于对方所引论文**（"若迭代根本不收敛则解法必有错"）：检索候选按可能性——
① Grayver & Kolev 2015 Geophysics（地学 hypre-AMS 奠基作）；② Rochlitz/Weiss 等 2022
Comput. Geosci.（复对称→实 2×2 块+外层 Krylov+内层 AMS，与本实现逐位对应）；
③ Smith 1996（散度校正一脉，说"变慢"非"不收敛"）；④ Axelsson & Kucherov 2000
（实块形式谱缺陷）。无论指哪篇，本实现按该检验是"收敛"的一类（34 步至 5e-13）。

### 2026-07-22 更正：对方所引文献已确认，此前候选猜测作废

对方所指的是**秦策博士论文《基于自适应矢量有限元法的三维大地电磁正反演研究》
（成都理工大学，119 页；同源期刊文献：秦策/王绪本/赵宁 2017 地球物理学报 60(6)）**，
不是此前猜的 Grayver/Rochlitz 等英文文献。全文精读结论（详见用户本地
~/Documents/秦策博士论文_三维大地电磁矢量有限元/阅读综述.md）：

1. **"若不收敛则解法必有错"在论文中无逐字原文**。原文说的是：通用预条件
   （ILU/SOR/Jacobi/BiCG）解复系数系统"多数情况下不能收敛"；而其推荐方案
   ——实对称块 (3-2) [Kr,-Ki;-Ki,-Kr]·(er,ei)=(sr,-si) + FGMRES + 分块对角预条件
   （内层 LDLT 或 hypre AMS）——外层 16-22 步（≤25 步到 1e-8）、内层 2-5 步，
   且对频率/自由度/局部加密不敏感（条件数 ≤√2 有证明）。
2. **该推荐方案与本求解器逐项同构**（同一实形式、FGMRES、hypre AMS、PETSc），
   我们 16 位数据实测（外层 19 步到 1e-7 / 34 步到 5e-13，内层 3-4 步）
   **落在论文自己的性能包络内**——按对方所引文献自己的标准，本实现是"正确"的一类。
   规模指引（DoFs>50 万用迭代+AMS）也恰好覆盖本算例（136 万实自由度）。
3. **边长归一化不是论文约定**：论文第 2 章为标准 Whitney/Nédélec 环量自由度，
   无 diag(len) 缩放记载。对方数据的缩放是实现层偏离，-fortran_upper_unscale_edge_len
   正是变回论文约定。可向对方求证其代码在哪一步引入了该归一化。
4. 论文验收全部用可观测量对比（解析解/Mackie/Nam/MARE2DEM），无"解向量全局 L2
   对比另一求解器"的先例——支持我们主张的验收口径。
5. 另：对方已于 2026-07-14 认可我们的结果（收敛精度认可；L2 差其自行对比
   与其之前结果误差 <1% 后亦认可）。
